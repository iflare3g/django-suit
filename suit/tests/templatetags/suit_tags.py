...
class SuitTagsTestCase(TestCase):
    ...
    def test_field_contents_foreign_linked(self):
        ...
        form = form_class(instance=city)
        ro_field = AdminReadonlyField(form, 'country', True, ma)

        result = field_contents_foreign_linked(ro_field)
        # Django 5.2+ AdminReadonlyField.contents() already links FK fields,
        # so field_contents_foreign_linked just passes through
        self.assertTrue(country.name in str(result))

        # Explicitly setting linked_readonly_fields should also work
        ro_field.model_admin.linked_readonly_fields = ('country',)
        result = field_contents_foreign_linked(ro_field)
        self.assertTrue(admin_url(country) in result)
